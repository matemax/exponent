...
Итак, по крайней мере за $d$ таких повторов мы либо убедимся, что $\sqrt{I_k}:G_d = (1)$
(и тогда надо будет перейти к следующему $d$),
либо получим простую компоненту этого идеала, не содержащую никаких обобщенных сепарант, и тогда по доказанному выше $I \ne (1)$.

\bigskip

Проведенные рассуждения записаны ниже в виде алгоритма.
Алгоритм был реализован авторами в системе компьютерной алгебры Sage.
