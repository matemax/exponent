...
\begin{lemma}
Пусть $f\in K\{y\}$ и $\ord f =l$, где $k$~--- поле нулевой характеристики. Рассмотрим идеал $[f]$, если он содержит квазилинейный многочлен, то $[f,\frac{\partial f}{\partial y_0},\ldots,\frac{\partial f}{\partial y_l}]=(1)$.
\end{lemma}

\begin{proof}
Пусть $g\in [f]$~---  квазилинейный многочлен порядка $\ord g = m$, тогда его можно расписать в виде $g=\sum\limits_{i=0}^ng_if^{(i)}=g'+y_m$, где $g'$ не зависит от переменных порядка выше, чем $m-1$. Предположим, что $n<2m-2l$, тогда будет выполнено, что $2k=2(n+l-m)<n$. А следовательно, по следствию~\ref{corollary:deriv_equal} и определению обобщённых сепарант мы можем для любого $m-l\leqslant p\leqslant n$  каждый многочлен $f^{(p)}$ представить в виде $f^{(p)}=\sum\limits_{i=0}^{k} S_{f,n,i} \, y_{p+l -i}+ Q$, где $k=p+l-m\,,\ord Q < p+l-k=m$. Все многочлены $f^{(j)}$, $j<m-l$ не зависят от переменных  $y_m$ и старше, следовательно, $\dfrac{\partial f^{(j)}}{\partial y_m}=0 $.

$$
1= \frac{\partial g}{\partial y_m}=\sum\limits_{i=0}^{n}\frac{\partial g_i}{\partial y_m}f^{(i)}+\sum\limits_{i=m-l}^{n}\frac{\partial f^{(i)}}{\partial y_m}g_i= \sum\limits_{i=0}^{n}\frac{\partial g_i}{\partial y_m}f^{(i)}+\sum\limits_{i=m-l}^{n}S_{f,i,i+l-m}g_i\in [f,\frac{\partial f}{\partial y_0},\ldots,\frac{\partial f}{\partial y_l}].
$$

Вторая часть суммы принадлежит идеалу $ [f,\frac{\partial f}{\partial y_0},\ldots,\frac{\partial f}{\partial y_l}]$, т.к.  $S_{f,n,k}=\sum\limits_{j=0}^{l}C_n^{k - l + j
}\left(\frac{\partial f}{\partial y_j}\right)^{(k - l + j)}$ по определению $S_{f,n,k}$.

Если $n>2m-2l$, то мы иожем продифференцировать $g$ $k$ раз ($k$ определим позже), от этого многочлен не перестанет быть квазилинейным, $m'=m+k$, $n'=n+k$. Возьмём $k$, чтобы выполнялось неравенство $n'>2m'-2l$.

\end{proof}

Заметим, что когда мы решали задачу про равенство $[f]+(h_1,\ldots,h_t)=1$, мы могли выбрать в качестве $ (h_1,\ldots,h_t)=(f)$. Тогда конечно $[f]+(f)=[f]\ne (1)$. Но мы можем определить для этого идеала все обозначения введённые нами в параграфе 3. Обозначения в следующей лемме взяты как раз таким путём.

\begin{corollary}
Пусть $f\in K\{y\}$ и $[f,\frac{\partial f}{\partial y_0},\ldots,\frac{\partial f}{\partial y_l}]=(1)$. Рассмотрим идеал $[f]+(f)=[f]\subseteq K\{y\}$. Предположим, что для некоторого  $n_0\ge\max\{2p+1,\ord{h_1},\ldots,\ord{h_t},l,w\}$  и для любых $k,i$ верно, что 
$$
\psi(g_{k,n},\Pi_{n_0+l}(V^{n_0}_{k,i}))\leqslant n_0+k.
$$ 

То $\sqrt{[f]}\cap K[y_0,\ldots,y_{n_0+l}]=\mathbf{I}(\bigcup\limits_{i,k}(V^{n_0}_{k,i}))$.
\end{corollary}

\begin{proof}
ПРедположим, что это не так. Тогда существует $g\in \sqrt{[f]}\cap K[y_0,\ldots,y_{n_0+l}]$, который не зануляется  на  хотя бы одном многообразии $(V^{n_0}_{k_0,i_0}))$. Из этого следует, что из точек  $(V^{n_0}_{k_0,i_0}))$ до решения системы $f=\ldots=f^{(i)}=\ldots=0$ мы можем продлить точки многообразия $U=(V^{n_0}_{k_0,i_0}))\cap \mathbf{V}(g)\subseteq K^{n_0+l+1}$. Но $U$ является многообразием меньшей размерности, чем $(V^{n_0}_{k_0,i_0})$, но это противоречит теореме~\ref{theorem:general th1}.
\end{proof}
\begin{remark}
Это следствие показывает, что с некоторого момента, а именно с $n_0$  пересечения  разикала идеала $(f,\ldots,f^{(n)})\subseteq K\{y\}$ с кольцом многочленов $K[y_0,\ldots,y_{n_0+l}]$ будет постоянным. Из теоремы гильберта о базисе мы знаем,  что такой момент должен был наступить, иначе у нас бы была бесконечная возрастающая цепочка идеалов, но  мы не могли определить, когда он наступит.  
\end{remark}
