...
Тогда для любых $k$ и $n$ с условием  $n\in\mathbb{N},0\leqslant k\in\mathbb{Z}$, $n>2k$ верно следующее
равенство:
$$
...
Пусть $f$~--- дифференциальный многочлен, $\ord f = l$ и $n\in\mathbb{N},0\leqslant k\in\mathbb{Z}$, $n>2k$.
Тогда
\begin{enumerate}
...
\end{proof}
\begin{definition}
Пусть $f\in\mathbf{K}\{y\},\ord f=l$  и $n\in\mathbb{N},0\leqslant k\in\mathbb{Z}$ такие, что $n>2k$. \emph{Обобщёнными сепарантами} многочлена $f$ будем назвать многочлены $S_{f,n,k}=\sum\limits_{j=0}^{l}C_n^{k - l + j
