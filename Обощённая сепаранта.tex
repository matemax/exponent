...
Дадим несколько необходимых определений и обозначений.
...
идеалы, порожд\"енные элементами, заключенными внутри скобок.
...
По следствию~\ref{corollary:deriv_equal} и определению обобщ\"енных сепарант мы можем для любого $m-l\leqslant p\leqslant n$  каждый многочлен $f^{(p)}$ представить в виде $f^{(p)}=\sum\limits_{i=0}^{k} S_{f,p,i} \, y_{p+l -i}+ Q$, где $k=p+l-m$, $\ord Q < p+l-k=m$.
...
Если $n>2m-2l$, то мы можем продифференцировать $g$ $k$ раз ($k$ определим позже), от этого многочлен не перестанет быть квазилинейным, $m'=m+k$, $n'=n+k$. Возьм\"ем $k$, чтобы выполнялось неравенство $n'<2m'-2l$. Лемма доказа.

\end{proof}

\medskip
Также с использованием обобщ\"енных сепарант нам удалось решить следующую задачу: дан дифференциальный многочлен $f\in K\{y\}$ с условием $[f,S_f]=(1)$, требуется проверить верно ли равенство $[f]+(S_f)=(1)$ или нет. Как доказал Трушин в работе~\cite{Trushin}, при условии $[f,S_f]=(1)$  равенство идеала $[f]+(S_f)$ единичному эквивалентно существованию в идеале $[f]$  квазилинейного многочлена. 

Точно такое же условие возникает у Колчина в работе~\cite{E. R. Kolchin}. Но как алгоритмически проверять  это условие было непонятно. Но это уже отдельная тема. Колчин вв\"ел понятие экспоненты дифференциального идеала $I$ (наименьшее такое $m$, что  радикал  идеала $I$ в степени $m$ принадлежит $I$). Колчин рассматривал идеалы, порожд\"енные одним многочленом $f$ первого порядка. Он вс\"е разделил на два случая $[f,S_f]=(1)$ и $[f,S_f]\ne(1)$. В первом случае, Колчин доказал, что если $[f]+(S_f)=(1)$, то экспонента равна 1. 
...
Фундаментальная и прикладная математика, г. 2007, том 13, вып 1, 215--227.


\bibitem{E. R. Kolchin}
E. R. Kolchin.
\emph{On the Exponents of Differential Ideals.}
The Annals of Mathematics, 1941, Second Series, Volume 42, Issue 3 , 740--777.
